...
class GameTable(object):
    ...
    def __init__(self, _input_deck=DeckOfCards(0)):
        ...
        self.start_hash = hash(self.start_deck)
        self.play_off = []
    ...
    def check_table(self):
        ...
        # for _tempI in self.play_off:
        #     _x = Card(64)
        #     for _tempJ in _tempI.list_of_cards:
        #         if _x != Card(64):
        #             if (_tempJ.suit() == _x.suit() and _tempJ.next_card_for(_x)) or  \
        #                     (_x.range() == 0 and _tempJ.range() == 12) or \
        #                     (_tempJ.range() == 12 and _tempJ == _tempI.see_end_card()):
        #                 _message = _message + _x.__str__()
        #         _x = _tempJ

        if GLOBAL_TRACE:
            print('карт->', _all_card, 'сумма->', _all_sum, '; снос->', len(self.move_history), 'ходов->', self.count_moves)
            # print(_message)
        return
...
# ++++++
            ...
            for _tempJ in self.play_visible:
                ...
                if _tempJ != _deck_from:
                    # проверим, что лежит первой в стопке, чтобы цикл предотвратить
                    if _card_from == _deck_from.see_end_card():
                        # разный цвет и следующая
                        if (not _card_from.same_color(_x)) and _card_from.next_card_for(_x) and _x.range() != 12:
                            self.add_move('vis', self.play_visible.index(_deck_from),
                                          _deck_from.list_of_cards.index(_card_from), _card_from,
                                          'vis', self.play_visible.index(_tempJ),
                                          _tempJ.list_of_cards.index(_x), _x)

                    # проверка возможности сноса Короля на пустое место
                    if _card_from != _deck_from.see_end_card() \
                            and _card_from.range() == 11 and _x.number() == 63 \
...
if __name__ == '__main__':
    ...
    while len(j.active_move) > 0 or j.count_stack != 0:
        print(j)
        j.search_moves()
        j.do_move()
        j.check_table()
